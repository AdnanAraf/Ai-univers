...
const displayData = (data, dataLimite, dateSort) => {
  ...
  data.forEach((element) => {
    ...
    const newDate = `${("0" + d.getDate()).slice(-2)}/${(
      "0" +
      (d.getMonth() + 1)
    ).slice(-2)}/${d.getUTCFullYear()}`;
    ...
    div.innerHTML = `
        <div class="card p-3" id="${element.features}">
            <img src="${
              element.image
            }" class="img-fluid img-thumbnail rounded-4" alt="...">
            <div class="card-body">
            ...
            <ol class="text-muted">
                ...
                <li>${
                  element.features[2] ? element.features[2] : "No Feature"
                }</li>
            </ol>
            <hr>
            ...
            <div class="d-flex justify-content-between">
                ...
                <button onclick="showDetails('${
                  element.id
                }')" type="button" data-bs-toggle="modal" data-bs-target="#aiModel" class="btn border rounded-full rounded-pill"><i class="text-muted bi bi-arrow-right"></i></button>
            </div>
            </div>
        </div>
        `;
...
const innerSetText = (idName, innerValue = "not found") => {
  innerValue =
    innerValue === "0"
      ? "Free of Cost/"
      : innerValue == "Contact us for pricing"
      ? "Free of Cost/"
      : innerValue === "Free"
      ? "Basic"
      : innerValue;
...
const inputOutputExamples = (
  idName,
  innerValue = "Can you give any example?"
) => {
  const variable = document.getElementById(idName);
  variable.innerText = innerValue;
};
